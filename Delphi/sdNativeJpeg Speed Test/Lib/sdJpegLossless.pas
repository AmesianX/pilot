...
uses
  Classes, SysUtils, Graphics, sdJpegTypes;
...
uses
  sdJpegFormat, sdJpegBlockCoder, sdJpegConsts;
...
begin
  ...
  // Indicate that the samples are no longer valid (requires a new IDCT to get them)
  Coder.HasSamples := False;
  Info := TsdJpegFormat(FOwner).CodingInfo;
  if ALeft < 0 then ALeft := 0;
  if ATop < 0 then ATop := 0;
  if ARight > Info.Width then ARight := Info.Width;
  if ABottom > Info.Height then ABottom := Info.Height;
  L := (ALeft div Info.McuWidth);
  T := (ATop div Info.McuHeight);
  ALeft := L * Info.McuWidth;
  ATop := T * Info.McuHeight;
  // Determine number of mcu blocks to copy
  W := (ARight - ALeft + Info.McuWidth - 1) div Info.McuWidth;
  H := (ABottom - ATop + Info.McuHeight - 1) div Info.McuHeight;
  if (W <= 0) or (H <= 0) then exit;
  // Update Info
  Info.HorzMcuCount := W;
  Info.VertMcuCount := H;
  Info.Width := ARight - ALeft;
  Info.Height := ABottom - ATop;
  // Crop each map
...
begin
  Result := nil;
  if not (FOwner is TsdJpegFormat) then exit;
  FCoder := TsdJpegFormat(FOwner).Coder;
  if not (FCoder is TsdJpegBlockCoder) or not FCoder.HasCoefs then
    raise EInvalidGraphic.Create(sNoDCTCoefficentsAvailable);
...
begin
  Result := 1;
  Space := TsdJpegFormat(FOwner).DetectStoredColorSpace;
  case Space of
  jcRGB, jcRGBA, jcCMYK:
    Result := 3;
  end;
...
begin
  ...
  Temp := Info.HorzSamplingMax;
  Info.HorzSamplingMax := Info.VertSamplingMax;
  Info.VertSamplingMax := Temp;
  // swap mcuwidth/height
  Temp := Info.McuWidth;
  Info.McuWidth := Info.McuHeight;
  Info.McuHeight := Temp;
  // Make sure to match the restart interval (if any) to new horz mcu count
  if Info.RestartInterval = Info.HorzMcuCount then
    Info.RestartInterval := Info.VertMcuCount;
  // swap horz/vert mcu count
  Temp := Info.HorzMcuCount;
  Info.HorzMcuCount := Info.VertMcuCount;
  Info.VertMcuCount := Temp;
  // We must also transpose quantization tables, since some software writes
  // non-axisymmetric quant tables
  for i := 0 to Info.QuantizationTables.Count - 1 do
    Info.QuantizationTables[i].Transpose;
end;
...
begin
  ...
  if not AMap.HasCoefBackup then
    AMap.MakeCoefBackup;
  // Quantization table
  Table := TsdJpegFormat(FOwner).CodingInfo.QuantizationTables[AMap.Frame.QTable];
  Quant := Table.Quant[0];
...
begin
  Frame := AMap.Frame;
  X := AMcuX * Frame.HorzSampling;
  Y := AMcuY * Frame.VertSampling;
  W := AMcuWidth * Frame.HorzSampling;
  H := AMcuHeight * Frame.VertSampling;
...
begin
  ...
  begin
    ...
    Frame := Map.Frame;
    Temp := Frame.HorzSampling;
    Frame.HorzSampling := Frame.VertSampling;
    Frame.VertSampling := Temp;
...
begin
  ...
  TsdJpegFormat(FOwner).Markers.RemoveMarkers([mkDHT]);
  Info := TsdJpegFormat(FOwner).CodingInfo;
  Info.DCHuffmanTables.clear;
  Info.ACHuffmanTables.clear;
end;
