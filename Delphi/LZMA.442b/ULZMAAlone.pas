...
begin
  Result := Copy(str, offset, Length(str));
end;

function GetInt(const str: string; const offset: integer): integer;
begin
  Result := StrToInt(GetStr(str, offset));
end;
...
begin
  ...
  case s[1] of
    ...
    'e':
      begin
        if (l >= 3) and (s[2] = 'o') and (s[3] = 's') then
        begin
          Eos := true;
          result := true;
        end;
...
begin
  ...
  begin
    ...
    begin
      if command = kBenchmark then
      begin
        try
          NumBenchMarkPasses := strtoint(s);
          if NumBenchMarkPasses < 1 then
          begin
            result := false;
            exit;
          end;
...
begin
  writeln(#10'Usage:  LZMA <e|d> [<switches>...] inputFile outputFile'#10 +
    '  e: encode file'#10 + '  d: decode file'#10 + '  b: Benchmark'#10 +
    '<Switches>'#10 +
...
begin
  ...
  if not params.Parse then
  begin
    writeln(#10'Incorrect command');
    exit;
  end;
  if params.command = kBenchmark then
  begin
    dictionary := 1 shl 21;
  ...
  else if (params.command = kEncode) or (params.command = kDecode) then
  begin
    inStream := TBufferedFS.Create(params.InFile, fmOpenRead or
      fmsharedenynone);
    ...
    if params.Eos then
      Eos := true;
    if params.command = kEncode then
    begin
      encoder := TLZMAEncoder.Create;

//      encoder.SetAlgorithm(0);
//      encoder.SetDictionarySize(0);
//      encoder.SetDictionarySize(1 shl 30);
//      encoder.SeNumFastBytes(5);
    ...
    raise Exception.Create('Incorrect command');
  params.Free;
end;
