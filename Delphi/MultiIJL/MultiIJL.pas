...
uses
  SystemFolder, 
  Windows, SysUtils, Classes, Graphics, IJL;
...
const
  OriginalDllName : String = 'Ijl15.dll';
type
  TMultiIJL = class
  private
    DLLHandle : THandle;
  ...
  public
    ...
    ijlErrorStr : function(Code : Integer) : PChar; stdcall;

    procedure BitmapStreamToJpeg(BitmapStream: TStream; Dst:TStream; Quality:integer=100); overload;
    procedure BitmapStreamToJpeg(BitmapStream: TMemoryStream; Dst: TStream; AWidth, AHeight : Integer;
      Quality: integer=100; APixelFormat : TPixelFormat = pf32bit); overload;
...
constructor TMultiIJL.Create(DllName: String = '');
begin
  FisLoaded := false;

  if Trim(DllName) = '' then begin
    DllName := Format('%s\%d%d.dll', [SystemFolder.TempFolder, Random(MaxInt), GetTickCount]);
    CopyFile(PChar(OriginalDllName), PChar(DllName), False);
  end;

  DLLHandle := LoadLibraryA(PChar(DllName));
  if DLLHandle < 32 then begin
    raise Exception.Create('Dll LoadError');
  end;

  @ijlInit := GetProcAddress(DllHandle, PChar('ijlInit'));
  if @ijlInit = nil then raise Exception.Create('Dll LoadError');

  @ijlFree := GetProcAddress(DllHandle, PChar('ijlFree'));
  if @IjlFree = nil then raise Exception.Create('Dll LoadError');

  @ijlRead := GetProcAddress(DllHandle, PChar('ijlRead'));
  if @ijlRead = nil then raise Exception.Create('Dll LoadError');

  @ijlWrite := GetProcAddress(DllHandle, PChar('ijlWrite'));
  if @ijlWrite = nil then raise Exception.Create('Dll LoadError');

  @ijlGetLibVersion := GetProcAddress(DllHandle, PChar('ijlGetLibVersion'));
  if @ijlGetLibVersion = nil then raise Exception.Create('Dll LoadError');

  @ijlErrorStr := GetProcAddress(DllHandle, PChar('ijlErrorStr'));
  if @ijlErrorStr = nil then raise Exception.Create('Dll LoadError');
...
procedure TMultiIJL.JpegToBitmap(Src: TStream; Bitmap: TBitmap);
var
...
begin
  ...
  GetMem(SrcBuffer, Src.Size);
  try
    Src.Position := 0;
    Src.Read(SrcBuffer^, Src.Size);
    try
      if ErrorCode <> IJL_OK then begin
        raise Exception.Create('Jpeg Error');
        Exit;
      end;
      IJLData.JPGFile := nil;
      IJLData.JPGBytes := SrcBuffer;
      IJLData.JPGSizeBytes := Src.Size;
      ...
      try
        Stream.Size := (IJLData.JPGWidth * IJLData.JPGHeight * IJLData.DIBChannels);// + sizeof(BITMAPFILEHEADER);
        IJLData.DIBWidth := IJLData.JPGWidth;
        IJLData.DIBHeight := -IJLData.JPGHeight;
        IJLData.DIBChannels := 3;
        IJLData.DIBPadBytes := 0;
        IJLData.DIBBytes := Stream.Memory;
        ...
        Stream.Position := 0;
        Bitmap.Height := -IJLData.DIBHeight;
        Bitmap.Width := IJLData.DIBWidth;
        Bitmap.PixelFormat := pf24bit;
        Move(Stream.Memory^, Bitmap.ScanLine[Bitmap.Height-1]^, Bitmap.Width * Bitmap.Height * 3);
      finally
        Stream.Free;
      end;
    finally
...
procedure TMultiIJL.BitmapStreamToJpeg(BitmapStream: TStream; Dst: TStream;
  Quality: integer);
var
  Bitmap : TBitmap;
  IJLData: TJPEG_CORE_PROPERTIES;
  ...
  try
    ...
    try
      try
        ...
        IJLData.DIBPadBytes := 0;
        IJLData.Pad := 0;
        IJLData.DIBWidth := Bitmap.Width;
        IJLData.DIBHeight := -Bitmap.Height;
        IJLData.DIBBytes := Bitmap.ScanLine[Bitmap.Height - 1];
        IJLData.DIBColor := IJL_BGR;
        GetMem(JpegBuffer, 3 * Bitmap.Width * Bitmap.Height);
        try
          IJLData.JPGWidth := Bitmap.Width;
          IJLData.JPGHeight := Bitmap.Height;
          IJLData.JPGFile := nil;
          IJLData.JPGBytes := JpegBuffer;
          IJLData.JPGSizeBytes := 3 * Bitmap.Width * Bitmap.Height * 2;
          IJLData.JPGChannels := 3;
          IJLData.JPGColor := IJL_YCBCR;
          IJLData.JPGSubsampling := IJL_411;
          IJLData.jquality := 100;
          ...
          Dst.Write(IJLData.JPGBytes^, IJLData.JPGSizeBytes);
          Dst.Position := 0;
        finally
          FreeMem(JpegBuffer);
        end;
...
procedure TMultiIJL.BitmapStreamToJpeg(BitmapStream: TMemoryStream;
  Dst: TStream; AWidth, AHeight, Quality: integer; APixelFormat: TPixelFormat);
var
  IJLData: TJPEG_CORE_PROPERTIES;
  ErrorCode : Integer;
  JpegBuffer : Pointer;
...
begin
  TempStream := nil;
  ZeroMemory(@IJLData, sizeof(IJLData));
  ijlInit(@IJLData);
  try
    case APixelFormat of
      pf8bit : IJLData.DIBChannels := 1;
      pf15bit, pf16bit : IJLData.DIBChannels := 2;
      pf24bit : IJLData.DIBChannels := 3;
      pf32bit : begin
        IJLData.DIBChannels := 3;
        TempStream := TMemoryStream.Create;
        TempStream.SetSize(AWidth * AHeight * 3);
        Bitmap32to24(Pointer(Cardinal(BitmapStream.Memory) + 54),
          TempStream.Memory, AWidth, AHeight);
        BitmapStream := TempStream;
      end;
    end;
    IJLData.DIBPadBytes := 0;
    IJLData.DIBWidth := AWidth;
    IJLData.DIBHeight := -AHeight;
    IJLData.DIBBytes := BitmapStream.Memory;
    IJLData.DIBColor := IJL_BGR;
    ...
    try
      ...
      IJLData.JPGColor := IJL_YCBCR;
      IJLData.JPGSubsampling := IJL_411;
      IJLData.jquality := Quality;

      ErrorCode := ijlWrite(@IJLData, IJL_JBUFF_WRITEWHOLEIMAGE);
      if ErrorCode <> IJL_OK then begin
        raise Exception.Create(Format('Jpeg Error Code : %d', [ErrorCode]));
        Exit;
      end;
...
procedure TMultiIJL.BitmapToJpeg(Bitmap:TBitmap; Dst:TStream; Quality:integer=100);
var
  IJLData : TJPEG_CORE_PROPERTIES;
  JpegBuffer : Pointer;
  BBitmap : TBitmap;
...
begin
  ...
  try
    ...
    BBitmap.Width := Bitmap.Width;
    BBitmap.Height := Bitmap.Height;
    BBitmap.Canvas.Draw(0,0, Bitmap);
    ZeroMemory(@IJLData, sizeof(IJLData));
    ijlInit(@IJLData);
    try
      try
        ...
        IJLData.DIBPadBytes := 0;
        IJLData.Pad := 0;
        IJLData.DIBWidth := BBitmap.Width;
        IJLData.DIBHeight := -BBitmap.Height;
        IJLData.DIBBytes := BBitmap.ScanLine[BBitmap.Height - 1];
        IJLData.DIBColor := IJL_BGR;
        GetMem(JpegBuffer, 3 * BBitmap.Width * BBitmap.Height);
        try
          IJLData.JPGWidth := BBitmap.Width;
          IJLData.JPGHeight := BBitmap.Height;
          IJLData.JPGFile := nil;
          IJLData.JPGBytes := JpegBuffer;
          IJLData.JPGSizeBytes := 3 * BBitmap.Width * BBitmap.Height * 2;
          IJLData.JPGChannels := 3;
          IJLData.JPGColor := IJL_YCBCR;
          IJLData.JPGSubsampling := IJL_411;
          IJLData.jquality := 100;
